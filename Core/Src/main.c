...
double calcThermistorTemp(void) {
	uint16_t adcResolution = 4095;
//	uint16_t referenceVoltage = 3238;  // mV
	uint16_t serialRezistance = 10570; // ohm
	uint16_t RawADC = 0;
	...
	//	uint16_t rawVoltage = (RawADC * referenceVoltage) / adcResolution;
	...
	float resistance = serialRezistance * RawADC / (adcResolution - RawADC);
	/*
	 * Rther = 10000 * ADC / (4095 - 10000)
	 * 4095 = 12 bit resolution ADC
	 * 10000 Serial integrated 10k resistor
	 */
...
//			HAL_GPIO_TogglePin(GPIOB, GPIO_PIN_11);
			...
			if (action > 0) {
				__HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, (1000 * action - s1));
			} else {
...
//			double temp = calcThermistorTemp();
////
//			float action = PI_Compute(&pid, 20, temp);
...
//			sprintf(buffer, "A: %d.%d", action_int / 10, abs(action_int % 10));
//			I2C_LCD_WriteString(0, buffer);
		}
		if (programCountdown == 1 && tickCounter == 0) {
			countdown--;

			sprintf(buffer, "%d:%02d", countdown / 60, countdown % 60);
			I2C_LCD_SetCursor(0, 7, 0);
			I2C_LCD_WriteString(0, buffer);
...
//			action = PI_Compute(&pid, 50, temp);

//			HAL_GPIO_WritePin(GPIOB, GPIO_PIN_11, (action > 0) ? SET : RESET);

//			if (action > 0) {
//				__HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, (10000 * action));
//			} else {
//				__HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, 0);
//			}
		}
		if (counter % 5 == 0) {
			I2C_LCD_SetCursor(0, 8, 0);
			int tempset_int = (int) (setTemp );
			sprintf(buffer, "T:%d.%dC", tempset_int);
			I2C_LCD_WriteString(0, buffer);
